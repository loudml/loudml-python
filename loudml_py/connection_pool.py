...
class ConnectionSelector(object):
    ...
    *currently* live connections to choose from.
    ...
    :class:`~loudml.Transport` as `hosts` param and the same that is
...
class ConnectionPool(object):
    """
    Container holding the :class:`~loudml.Connection` instances,
    managing the selection process (via a
    :class:`~loudml.ConnectionSelector`) and dead connections.

    It's only interactions are with the :class:`~loudml.Transport` class
    ...
    def __init__(self, connections, dead_timeout=60, timeout_cutoff=5,
        ...
        :arg connections: list of tuples containing the
            :class:`~loudml.Connection` instance and it's options
        ...
        :arg selector_class: :class:`~loudml.ConnectionSelector`
    ...
    def mark_dead(self, connection, now=None):
        ...
        try:
            ...
            timeout = self.dead_timeout * \
                2 ** min(dead_count - 1, self.timeout_cutoff)
