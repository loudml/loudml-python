...
class Urllib3HttpConnection(Connection):
    ...
    :arg port: port to use (integer, default: 8077)
    :arg url_prefix: optional url prefix for loudml
    ...
    def __init__(
        self, host='localhost', port=8077, http_auth=None,
        ...
        # if providing an SSL context
        # raise error if any other SSL related flag is used
        if ssl_context and ((verify_certs is not VERIFY_CERTS_DEFAULT)
                            or ca_certs or client_cert or client_key or ssl_version):
            warnings.warn(
    ...
    def perform_request(self, method, url, params=None, body=None, timeout=None, ignore=(), headers=None):
        ...
        try:
            ...
            response = self.pool.urlopen(method, url, body, retries=Retry(
                False), headers=request_headers, **kw)
            duration = time.time() - start
            raw_data = response.data.decode('utf-8')
        except Exception as e:
            self.log_request_fail(method, full_url, url,
                                  body, time.time() - start, exception=e)
