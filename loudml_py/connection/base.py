...
# create the loudml.trace logger, but only set propagate to False if the
# logger hasn't already been configured
_tracer_already_configured = 'loudml.trace' in logging.Logger.manager.loggerDict
tracer = logging.getLogger('loudml.trace')
...
class Connection(object):
    """
    Class responsible for maintaining a connection to a Loud ML node. It
    holds persistent connection pool to it and it's main interface
    (`perform_request`) is thread-safe.

    Also responsible for logging.
    """
    def __init__(self, host='localhost', port=8077, use_ssl=False, url_prefix='', timeout=10, **kwargs):
        """
    ...
    def _log_trace(self, method, path, body, status_code, response, duration):
        ...
        tracer.info("curl %s-X%s 'http://localhost:8077%s' -d '%s'",
